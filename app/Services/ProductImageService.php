...
class ProductImageService
{
    ...
    public const MAX_SIZE = 5242880; // 5 MB
    ...
    public function url(?string $path): ?string
    {
        ...
        foreach (['webp', 'jpg', 'jpeg', 'png'] as $extension) {
            ...
            if ($alternate && Storage::disk('public')->exists($alternate)) {
                return '/storage/'.ltrim($alternate, '/');
            }
        }
    ...
    private function storeRaw(UploadedFile $file): string
    {
        ...
        $relativePath = 'products/'.Str::uuid()->toString().'.'.$extension;

        Storage::disk('public')->putFileAs('products', $file, basename($relativePath));
    ...
    private function loadImage(UploadedFile $file)
    {
        ...
        if ($source === false) {
            throw new RuntimeException('Не удалось обработать изображение. Проверьте формат файла (JPG, PNG).');
        }
